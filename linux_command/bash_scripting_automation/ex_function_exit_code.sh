...
function backup_file () {
  ...
  if [ -f "$1" ]
  then
    # Make the BACKUP_FILE variable.
    # $$ is special variable means the current process id
    local BACKUP_FILE="/tmp/$(basename ${1}).$(date +%F).$$"
    echo "Backing up $1 to ${BACKUP_FILE}"
    # The exit status of the function will be the
...
# You could have put this functionality inside the
